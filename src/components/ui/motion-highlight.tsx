...
import { a, useTransition, type SpringConfig } from "@react-spring/web";
...
function MotionHighlight<T extends string>({
  ...
  const {
    ...
    onValueChange,
    className,
    transition = { tension: 350, friction: 35 },
    hover = false,
    enabled = true,
    controlledItems,
    disabled = false,
  ...
  const render = React.useCallback(
    (children: React.ReactNode) => {
      const trans = useTransition(boundsState, {
        from: {
          top: boundsState?.top,
...
function MotionHighlightItem({
  ...
  const trans = useTransition(isActive && !isDisabled, {
    from: {
      opacity: 0,
    },
    enter: {
    ...
    leave: {
      ...
      transition: {
        ...itemTransition,
        delay: 0 + (exitDelay ?? contextExitDelay ?? 0),
      },
    },
  });
