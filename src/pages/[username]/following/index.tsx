import { useGetFollowingInfiniteQuery } from "@/app/features/relationship/relationship.api";
...
const FollowingPage = () => {
  ...
  useHeader(t("following_list.header"), {
    hideTitle: true,
  });
  const { profile } = useAuth();
  const { username } = useParams();
  const { data, fetchNextPage, hasNextPage, isLoading } =
    useGetFollowingInfiniteQuery(username!, {
      skip: !username,
    });
  ...
  if (isLoading)
    return (
      <div>
        <BigHeader>{t("following_list.header")}</BigHeader>
    ...
    return (
      <div>
        ...
        <Card className="md:px-4 gap-2 h-40 justify-center items-center">
          <span className="text-center text-muted-foreground animated transition-colors block">
            {t(
              `following_list.empty.${profile?.username === username ? "mine" : "other"}`,
            )}
          </span>
        </Card>
      </div>
